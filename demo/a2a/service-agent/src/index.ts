...
const RESOURCE_SERVER_URL = process.env.RESOURCE_SERVER_URL || 'http://localhost:5403';
  ...
  if (method === 'message/send') {
    ...
    if (skill === 'premium.summarize') {
      ...
      try {
        ...
        const resp = await callPremiumSummarize(resourceClient, body, clientPaymentHeader);
