...
if [ -f "$ROOT_DIR/demo/.env.local" ]; then
  set -a
  . "$ROOT_DIR/demo/.env.local"
  set +a
  echo "✅ Environment loaded from demo/.env.local"
else
  echo "⚠️  No demo/.env.local found, using defaults"
fi

# Set default environment variables
export PYTHON_SERVER_PORT=${PYTHON_SERVER_PORT:-5410}
